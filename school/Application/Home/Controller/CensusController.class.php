...
class CensusController extends AdminController {
    ...
    function __construct(){
        parent::__construct();
        $this->sid = I('get.sid') ? I('get.sid') : 3 ; // $this->error('错误页面！sid为空')
    ...
    public function machine_shend(){ //机械消费
		...
		$page = new \Think\Page($res['DTALIST']['PTOTAL'],$count);
    ...
    public function machine_operation(){ //机械运维考勤
        ...
        if( $date = I('get.date-range-picker')){
            $date = explode('-',trim($date) );
            $time['start'] = date('Y-m-d',strtotime($date[0]));
            $time['end'] = date('Y-m-d',strtotime($date[1]));
